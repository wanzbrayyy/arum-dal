(function () {
    ...
    function cacheDom() {
        [
            ...
            'customerName', 'customerPhone', 'orderNote', 'voucherCode', 'statusCard', 'statusBadge',
    ...
    function renderStatusCard(order) {
        ...
        els.statusText.textContent = `${t('queued')} ${activeOrder.tableName || ''}`.trim();
        els.latestOrderMeta.textContent = `${currency(activeOrder.pricing?.total || 0)} / ${activeOrder.paymentStatus || 'unpaid'}`;
        els.statusOrderCode.textContent = activeOrder.orderCode || '-';
        els.statusQueueInfo.textContent = `${activeOrder.queueNumber || '-'} / ${activeOrder.paymentStatus || 'unpaid'}`;
    }
